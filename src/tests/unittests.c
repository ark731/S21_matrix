/*
 * DO NOT EDIT THIS FILE. Generated by checkmk.
 * Edit the original source file "unit_tests/_unittests.check" instead.
 */

#include <check.h>
#include <math.h>
#include <stdlib.h>

#include "../s21_matrix.h"
#define CK_FORK 0

#define SUCCESS 1
#define FAILURE 0

void s21_init_matrix(double number, matrix_t *A) {
  for (int x = 0; x < A->rows; x += 1) {
    for (int y = 0; y < A->columns; number += 1.0, y += 1)
      A->matrix[x][y] = number;
  }
}

START_TEST(s21_calc_complements1) {
  int rows = 2;
  int columns = 2;
  matrix_t A = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &C);
  A.matrix[0][0] = 5;
  A.matrix[0][1] = 1;
  A.matrix[1][0] = 4;
  A.matrix[1][1] = 7;
  C.matrix[0][0] = 7;
  C.matrix[0][1] = -4;
  C.matrix[1][0] = -1;
  C.matrix[1][1] = 5;
  int ret = s21_calc_complements(&A, &R);
  ck_assert_int_eq(0, ret);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_calc_complements2) {
  int rows = 3;
  int columns = 3;
  matrix_t A = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &C);
  A.matrix[0][0] = 6;
  A.matrix[0][1] = 1;
  A.matrix[0][2] = 1;
  A.matrix[1][0] = 4;
  A.matrix[1][1] = 1;
  A.matrix[1][2] = 2;
  A.matrix[2][0] = 3;
  A.matrix[2][1] = 2;
  A.matrix[2][2] = 4;

  C.matrix[0][0] = 0;
  C.matrix[0][1] = -10;
  C.matrix[0][2] = 5;
  C.matrix[1][0] = -2;
  C.matrix[1][1] = 21;
  C.matrix[1][2] = -9;
  C.matrix[2][0] = 1;
  C.matrix[2][1] = -8;
  C.matrix[2][2] = 2;
  int ret = s21_calc_complements(&A, &R);
  ck_assert_int_eq(0, ret);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_calc_complements3) {
  int rows = rand() % 10 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, rows, &A);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < rows; j++) {
      double t = rand();
      A.matrix[i][j] = t / 7;
    }
  }
  int ret = s21_calc_complements(&A, &R);
  ck_assert_int_eq(0, ret);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
}
END_TEST

START_TEST(s21_calc_complements4) {
  int rows = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, rows + 1, &A);
  int ret = s21_calc_complements(&A, &R);
  ck_assert_int_eq(2, ret);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_calc_complements5) {
  int rows = 0;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns, &A);
  int ret = s21_calc_complements(&A, &R);
  ck_assert_int_eq(1, ret);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_calc_complements6) {
  double m[3][3] = {{1, 2, 3}, {0, 4, 2}, {5, 2, 1}};
  double r[3][3] = {{0, 10, -20}, {4, -14, 8}, {-8, -2, 4}};
  matrix_t matrix, res_org, res_my;
  s21_create_matrix(3, 3, &matrix);
  s21_create_matrix(3, 3, &res_org);
  for (int i = 0; i < matrix.rows; i++) {
    for (int j = 0; j < matrix.columns; j++) {
      matrix.matrix[i][j] = m[i][j];
    }
  }
  for (int i = 0; i < matrix.rows; i++) {
    for (int j = 0; j < matrix.columns; j++) {
      res_org.matrix[i][j] = r[i][j];
    }
  }
  int ret = s21_calc_complements(&matrix, &res_my);
  ck_assert_int_eq(ret, 0);
  ret = s21_eq_matrix(&res_org, &res_my);
  ck_assert_int_eq(ret, 1);
  s21_remove_matrix(&matrix);
  s21_remove_matrix(&res_my);
}
END_TEST

START_TEST(s21_calc_complements7) {
  matrix_t expected = {0};
  matrix_t m = {0};
  matrix_t result = {0};
  int code1 = s21_create_matrix(3, 3, &m);
  int code2 = s21_create_matrix(3, 3, &expected);
  if (code1 && code2) {
    m.matrix[0][0] = 1;
    m.matrix[0][1] = 2;
    m.matrix[0][2] = 3;

    m.matrix[1][0] = 0;
    m.matrix[1][1] = 4;
    m.matrix[1][2] = 2;

    m.matrix[2][0] = 5;
    m.matrix[2][1] = 2;
    m.matrix[2][2] = 1;

    expected.matrix[0][0] = 0;
    expected.matrix[0][1] = 10;
    expected.matrix[0][2] = -20;

    expected.matrix[1][0] = 4;
    expected.matrix[1][1] = -14;
    expected.matrix[1][2] = 8;

    expected.matrix[2][0] = -8;
    expected.matrix[2][1] = -2;
    expected.matrix[2][2] = 4;

    int code = s21_calc_complements(&m, &result);

    ck_assert_int_eq(s21_eq_matrix(&result, &expected), 1);
    ck_assert_int_eq(code, 0);
    s21_remove_matrix(&m);
    s21_remove_matrix(&result);
    s21_remove_matrix(&expected);
  }
}
END_TEST

START_TEST(s21_determinant1) {
  int rows = 1;
  int columns = 1;
  matrix_t A = {0};
  s21_create_matrix(rows, columns, &A);
  A.matrix[0][0] = 5;
  double det = 5;
  double res = 0;
  int ret = s21_determinant(&A, &res);
  ck_assert_int_eq(0, ret);
  ck_assert_double_eq(res, det);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant2) {
  int rows = 2;
  int columns = 2;
  matrix_t A = {0};
  s21_create_matrix(rows, columns, &A);
  A.matrix[0][0] = 5;
  A.matrix[0][1] = 1;
  A.matrix[1][0] = 4;
  A.matrix[1][1] = 7;
  double det = 31;
  double res = 0;
  int ret = s21_determinant(&A, &res);
  ck_assert_int_eq(0, ret);
  ck_assert_double_eq(res, det);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant3) {
  int rows = 3;
  int columns = 3;
  matrix_t A = {0};
  s21_create_matrix(rows, columns, &A);
  A.matrix[0][0] = 5;
  A.matrix[0][1] = 1;
  A.matrix[0][2] = 1;
  A.matrix[1][0] = 4;
  A.matrix[1][1] = 7;
  A.matrix[1][2] = 5;
  A.matrix[2][0] = 3;
  A.matrix[2][1] = 3;
  A.matrix[2][2] = 4;
  double det = 55;
  double res = 0;
  int ret = s21_determinant(&A, &res);
  ck_assert_int_eq(0, ret);
  ck_assert_double_eq(res, det);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant4) {
  int rows = 4;
  int columns = 4;
  matrix_t A = {0};
  s21_create_matrix(rows, columns, &A);
  A.matrix[0][0] = 1;
  A.matrix[0][1] = 1;
  A.matrix[0][2] = 1;
  A.matrix[0][3] = 5;
  A.matrix[1][0] = 6;
  A.matrix[1][1] = 7;
  A.matrix[1][2] = 8;
  A.matrix[1][3] = 6;
  A.matrix[2][0] = 3;
  A.matrix[2][1] = 2;
  A.matrix[2][2] = 3;
  A.matrix[2][3] = 3;
  A.matrix[3][0] = 0;
  A.matrix[3][1] = 2;
  A.matrix[3][2] = 0;
  A.matrix[3][3] = 2;
  double det = -44;
  double res = 0;
  int ret = s21_determinant(&A, &res);
  ck_assert_int_eq(0, ret);
  ck_assert_double_eq(res, det);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant5) {
  int rows = rand() % 10 + 1;
  matrix_t A = {0};
  s21_create_matrix(rows, rows, &A);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < rows; j++) {
      A.matrix[i][j] = 4;
    }
  }
  double det = 5;
  double res = 0;
  int ret = s21_determinant(&A, &res);
  ck_assert_int_eq(0, ret);
  ck_assert_double_ne(res, det);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant6) {
  int rows = rand() % 100 + 1;
  matrix_t A = {0};
  s21_create_matrix(rows, rows + 1, &A);
  double res = 0;
  int ret = s21_determinant(&A, &res);
  ck_assert_int_eq(2, ret);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant7) {
  int rows = rand() % 100 + 1;
  matrix_t A = {0};
  s21_create_matrix(rows, 0, &A);
  double res = 0;
  int ret = s21_determinant(&A, &res);
  ck_assert_int_eq(1, ret);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant8) {
  matrix_t matrix;
  s21_create_matrix(3, 3, &matrix);
  int count = 1;
  for (int i = 0; i < matrix.rows; i++) {
    for (int j = 0; j < matrix.columns; j++) {
      matrix.matrix[i][j] = count++;
    }
  }
  matrix.matrix[2][2] = 10;
  double res;
  int ret = s21_determinant(&matrix, &res);
  ck_assert_double_eq(res, -3.);
  ck_assert_int_eq(ret, 0);
  s21_remove_matrix(&matrix);
}
END_TEST

START_TEST(s21_eq_matrix1) {
  int rows = 0;
  int columns = rand() % 100 + 1;
  matrix_t M = {0};
  matrix_t N = {0};
  s21_create_matrix(rows, columns, &M);
  s21_create_matrix(rows, columns, &N);
  int ret = s21_eq_matrix(&M, &N);
  ck_assert_int_eq(FAILURE, ret);
  s21_remove_matrix(&M);
  s21_remove_matrix(&N);
}
END_TEST

START_TEST(s21_eq_matrix2) {
  matrix_t m = {0};
  const int rows = rand() % 100 + 1;
  const int cols = rand() % 100 + 1;
  s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  const int rows1 = rand() % 100 + 1;
  const int cols1 = rand() % 100 + 1;
  s21_create_matrix(rows1, cols1, &mtx);

  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      m.matrix[i][j] = rand() + 1;
    }
  }
  for (int i = 0; i < rows1; i++) {
    for (int j = 0; j < cols1; j++) {
      mtx.matrix[i][j] = rand();
    }
  }
  ck_assert_int_eq(s21_eq_matrix(&m, &mtx), 0);
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
}
END_TEST

START_TEST(s21_inverse_matrix1) {
  int rows = 1;
  int columns = 1;
  matrix_t A = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns, &A);
  A.matrix[0][0] = 5;
  int ret = s21_inverse_matrix(&A, &R);
  double inv = 0.2;
  ck_assert_int_eq(0, ret);
  ck_assert_double_eq(inv, R.matrix[0][0]);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
}
END_TEST

START_TEST(s21_inverse_matrix2) {
  int rows = 2;
  int columns = 2;
  matrix_t A = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &C);
  A.matrix[0][0] = 3;
  A.matrix[0][1] = 2;
  A.matrix[1][0] = 4;
  A.matrix[1][1] = 1;

  C.matrix[0][0] = -0.2;
  C.matrix[0][1] = 0.4;
  C.matrix[1][0] = 0.8;
  C.matrix[1][1] = -0.6;
  int ret = s21_inverse_matrix(&A, &R);
  ck_assert_int_eq(0, ret);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_inverse_matrix3) {
  int rows = 3;
  int columns = 3;
  matrix_t A = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &C);
  A.matrix[0][0] = 6;
  A.matrix[0][1] = 1;
  A.matrix[0][2] = 1;
  A.matrix[1][0] = 4;
  A.matrix[1][1] = 1;
  A.matrix[1][2] = 2;
  A.matrix[2][0] = 3;
  A.matrix[2][1] = 2;
  A.matrix[2][2] = 4;
  C.matrix[0][0] = 0;
  C.matrix[0][1] = 0.4;
  C.matrix[0][2] = -0.2;
  C.matrix[1][0] = 2;
  C.matrix[1][1] = -4.2;
  C.matrix[1][2] = 1.6;
  C.matrix[2][0] = -1;
  C.matrix[2][1] = 1.8;
  C.matrix[2][2] = -0.4;
  int ret = s21_inverse_matrix(&A, &R);
  ck_assert_int_eq(0, ret);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_inverse_matrix4) {
  int rows = 2;
  matrix_t A = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, rows, &A);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < rows; j++) {
      A.matrix[i][j] = 7;
    }
  }
  int ret = s21_inverse_matrix(&A, &R);
  ck_assert_int_eq(2, ret);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_inverse_matrix5) {
  int rows = 2;
  int columns = 2;
  matrix_t A = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &C);
  A.matrix[0][0] = 3;
  A.matrix[0][1] = 2;
  A.matrix[1][0] = 4;
  A.matrix[1][1] = 1;
  C.matrix[0][0] = 0;
  C.matrix[0][1] = 0;
  C.matrix[1][0] = 0.8;
  C.matrix[1][1] = -0.6;
  int ret = s21_inverse_matrix(&A, &R);
  ck_assert_int_eq(0, ret);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(FAILURE, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_inverse_matrix6) {
  int rows = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, rows + 1, &A);
  int ret = s21_inverse_matrix(&A, &R);
  ck_assert_int_eq(2, ret);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_inverse_matrix7) {
  int rows = 0;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns, &A);
  int ret = s21_inverse_matrix(&A, &R);
  ck_assert_int_eq(1, ret);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_inverse_matrix8) {
  double m[3][3] = {{2, 5, 7}, {6, 3, 4}, {5, -2, -3}};
  double r[3][3] = {{1, -1, 1}, {-38, 41, -34}, {27, -29, 24}};
  matrix_t matrix, res_org, res_my;
  s21_create_matrix(3, 3, &matrix);
  s21_create_matrix(3, 3, &res_org);
  for (int i = 0; i < matrix.rows; i++) {
    for (int j = 0; j < matrix.columns; j++) {
      matrix.matrix[i][j] = m[i][j];
    }
  }
  for (int i = 0; i < matrix.rows; i++) {
    for (int j = 0; j < matrix.columns; j++) {
      res_org.matrix[i][j] = r[i][j];
    }
  }
  int ret = s21_inverse_matrix(&matrix, &res_my);
  ck_assert_int_eq(ret, 0);
  ret = s21_eq_matrix(&res_org, &res_my);
  ck_assert_int_eq(ret, 1);
  s21_remove_matrix(&matrix);
  s21_remove_matrix(&res_my);
  s21_remove_matrix(&res_org);
}
END_TEST

START_TEST(s21_mult_matrix1) {
  int rowsA = 2;
  int columnsA = 1;
  int columnsB = 2;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rowsA, columnsA, &A);
  s21_create_matrix(columnsA, columnsB, &B);
  s21_create_matrix(rowsA, columnsB, &C);
  for (int i = 0; i < rowsA; i++) {
    for (int j = 0; j < columnsA; j++) {
      A.matrix[i][j] = i + 2;
    }
  }
  for (int i = 0; i < columnsA; i++) {
    for (int j = 0; j < columnsB; j++) {
      B.matrix[i][j] = j + 1;
    }
  }
  C.matrix[0][0] = 2;
  C.matrix[0][1] = 4;
  C.matrix[1][0] = 3;
  C.matrix[1][1] = 6;
  int ret = s21_mult_matrix(&A, &B, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_mult_matrix2) {
  int rowsA = rand() % 100 + 1;
  int columnsA = rand() % 100 + 1;
  int columnsB = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rowsA, columnsA, &A);
  s21_create_matrix(columnsA, columnsB, &B);
  s21_create_matrix(rowsA, columnsB, &C);
  for (int i = 0; i < rowsA; i++) {
    for (int j = 0; j < columnsA; j++) {
      double t = rand();
      A.matrix[i][j] = t / 3;
    }
  }
  for (int i = 0; i < columnsA; i++) {
    for (int j = 0; j < columnsB; j++) {
      double t = rand();
      B.matrix[i][j] = t / 5;
    }
  }
  for (int i = 0; i < C.rows; i++) {
    for (int j = 0; j < C.columns; j++) {
      for (int k = 0; k < A.columns; k++) {
        C.matrix[i][j] += A.matrix[i][k] * B.matrix[k][j];
      }
    }
  }
  int ret = s21_mult_matrix(&A, &B, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_mult_matrix3) {
  int rowsA = rand() % 100 + 1;
  int columnsA = rand() % 100 + 1;
  int columnsB = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rowsA, columnsA, &A);
  s21_create_matrix(columnsA, columnsB, &B);
  s21_create_matrix(rowsA, columnsB, &C);
  for (int i = 0; i < rowsA; i++) {
    for (int j = 0; j < columnsA; j++) {
      double t = rand();
      A.matrix[i][j] = t / 3;
    }
  }
  for (int i = 0; i < columnsA; i++) {
    for (int j = 0; j < columnsB; j++) {
      double t = rand();
      B.matrix[i][j] = t / 5;
    }
  }
  int temp = 0;
  for (int i = 0; i < C.rows; i++) {
    for (int j = 0; j < C.columns; j++) {
      for (int k = 0; k < A.columns; k++) {
        temp = A.matrix[i][k] * B.matrix[k][j] + 10;
        C.matrix[i][j] += temp;
      }
    }
  }
  int ret = s21_mult_matrix(&A, &B, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(FAILURE, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_mult_matrix4) {
  int rowsA = rand() % 100 + 1;
  int columnsA = rand() % 100 + 1;
  int columnsB = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  s21_create_matrix(rowsA, columnsA, &A);
  s21_create_matrix(columnsA + 1, columnsB, &B);
  int ret = s21_mult_matrix(&A, &B, &R);
  ck_assert_int_eq(2, ret);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_mult_matrix5) {
  int rowsA = 0;
  int columnsA = rand() % 100 + 1;
  int columnsB = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  s21_create_matrix(rowsA, columnsA, &A);
  s21_create_matrix(columnsA, columnsB, &B);
  int ret = s21_mult_matrix(&A, &B, &R);
  ck_assert_int_eq(1, ret);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_mult_number1) {
  double number = (rand() % 100 + 1) / 3;
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &C);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      double t = rand();
      A.matrix[i][j] = t / 2;
      C.matrix[i][j] = number * A.matrix[i][j];
    }
  }
  int ret = s21_mult_number(&A, number, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_mult_number2) {
  double number = (rand() % 100 + 1) / 3;
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &C);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      double t = rand();
      A.matrix[i][j] = t / 2;
      C.matrix[i][j] = number * A.matrix[i][j] * 2;
    }
  }
  int ret = s21_mult_number(&A, number, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(FAILURE, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_mult_number3) {
  double number = (rand() % 100 + 1) / 3;
  int rows = 0;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns, &A);
  int ret = s21_mult_number(&A, number, &R);
  ck_assert_int_eq(1, ret);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_sub_matrix1) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &B);
  s21_create_matrix(rows, columns, &C);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      double t = rand();
      A.matrix[i][j] = t / 3;
      B.matrix[i][j] = t / 5;
      C.matrix[i][j] = A.matrix[i][j] - B.matrix[i][j];
    }
  }
  int ret = s21_sub_matrix(&A, &B, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_sub_matrix2) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &B);
  s21_create_matrix(rows, columns, &C);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      double t = rand();
      A.matrix[i][j] = t / 3;
      B.matrix[i][j] = t / 5;
      C.matrix[i][j] = A.matrix[i][j] - B.matrix[i][j] + 1;
    }
  }
  int ret = s21_sub_matrix(&A, &B, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(FAILURE, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_sub_matrix3) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows + 1, columns, &B);
  int ret = s21_sub_matrix(&A, &B, &R);
  ck_assert_int_eq(2, ret);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_sub_matrix4) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns + 1, &A);
  s21_create_matrix(rows, columns, &B);
  int ret = s21_sub_matrix(&A, &B, &R);
  ck_assert_int_eq(2, ret);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_sub_matrix5) {
  int rows = 0;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &B);
  int ret = s21_sub_matrix(&A, &B, &R);
  ck_assert_int_eq(1, ret);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_sum_matrix1) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &B);
  s21_create_matrix(rows, columns, &C);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      double t = rand();
      A.matrix[i][j] = t / 3;
      B.matrix[i][j] = t / 5;
      C.matrix[i][j] = A.matrix[i][j] + B.matrix[i][j];
    }
  }
  int ret = s21_sum_matrix(&A, &B, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_sum_matrix2) {
  int rows = 0;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &B);
  int ret = s21_sum_matrix(&A, &B, &R);
  ck_assert_int_eq(1, ret);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_sum_matrix3) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows, columns, &B);
  s21_create_matrix(rows, columns, &C);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      double t = rand();
      A.matrix[i][j] = t / 3;
      B.matrix[i][j] = t / 5;
      C.matrix[i][j] = A.matrix[i][j] + B.matrix[i][j] + 1;
    }
  }
  int ret = s21_sum_matrix(&A, &B, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(FAILURE, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_sum_matrix4) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(rows + 1, columns, &B);
  int ret = s21_sum_matrix(&A, &B, &R);
  ck_assert_int_eq(2, ret);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_sum_matrix5) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns + 1, &A);
  s21_create_matrix(rows, columns, &B);
  int ret = s21_sum_matrix(&A, &B, &R);
  ck_assert_int_eq(2, ret);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_transpose_matrix1) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(columns, rows, &C);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      double t = rand();
      A.matrix[i][j] = t / 7;
      C.matrix[j][i] = t / 7;
    }
  }
  int ret = s21_transpose(&A, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(SUCCESS, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_transpose_matrix2) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  matrix_t C = {0};
  s21_create_matrix(rows, columns, &A);
  s21_create_matrix(columns, rows, &C);
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      double t = rand();
      A.matrix[i][j] = t / 7;
      C.matrix[j][i] = t / 7 + 1;
    }
  }
  int ret = s21_transpose(&A, &R);
  int res = s21_eq_matrix(&C, &R);
  ck_assert_int_eq(0, ret);
  ck_assert_int_eq(FAILURE, res);
  s21_remove_matrix(&A);
  s21_remove_matrix(&R);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(s21_transpose_matrix3) {
  int rows = 0;
  int columns = rand() % 100 + 1;
  matrix_t A = {0};
  matrix_t R = {0};
  s21_create_matrix(rows, columns, &A);
  int ret = s21_transpose(&A, &R);
  ck_assert_int_eq(1, ret);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_transpose_matrix4) {
  matrix_t mat_1, res_mat, res_org;

  s21_create_matrix(2, 2, &mat_1);
  s21_create_matrix(2, 2, &res_org);
  mat_1.matrix[0][0] = 1;
  mat_1.matrix[0][1] = 2;
  mat_1.matrix[1][0] = 3;
  mat_1.matrix[1][1] = 4;
  res_org.matrix[0][0] = 1;
  res_org.matrix[0][1] = 3;
  res_org.matrix[1][0] = 2;
  res_org.matrix[1][1] = 4;
  int ret = s21_transpose(&mat_1, &res_mat);
  ck_assert_int_eq(ret, 0);
  ret = s21_eq_matrix(&res_mat, &res_org);
  ck_assert_int_eq(ret, 1);
  s21_remove_matrix(&mat_1);
  s21_remove_matrix(&res_mat);
  s21_remove_matrix(&res_org);
}
END_TEST

START_TEST(s21_transpose_matrix5) {
  matrix_t mat_1, res_mat, res_org;
  double matrix[2][2] = {
      {1, 2},
      {3, 4},
  };
  double res[2][2] = {{1, 3}, {2, 4}};
  s21_create_matrix(2, 2, &mat_1);
  s21_create_matrix(2, 2, &res_org);
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      mat_1.matrix[i][j] = matrix[i][j];
    }
  }
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      res_org.matrix[i][j] = res[i][j];
    }
  }
  int ret = s21_transpose(&mat_1, &res_mat);
  ck_assert_int_eq(ret, 0);
  ret = s21_eq_matrix(&res_mat, &res_org);
  ck_assert_int_eq(ret, 1);
  s21_remove_matrix(&mat_1);
  s21_remove_matrix(&res_mat);
  s21_remove_matrix(&res_org);
}
END_TEST

START_TEST(s21_transpose_matrix6) {
  matrix_t matrix, res_mat;
  s21_create_matrix(2, 2, &matrix);
  matrix.matrix[0][1] = NAN;
  int ret = s21_transpose(&matrix, &res_mat);
  ck_assert_int_eq(ret, 1);
  s21_remove_matrix(&matrix);
}
END_TEST

START_TEST(s21_transpose_matrix7) {
  matrix_t matrix, res_mat;
  s21_create_matrix(2, 2, &matrix);
  matrix.matrix[0][1] = INFINITY;
  int ret = s21_transpose(&matrix, &res_mat);
  ck_assert_int_eq(ret, 1);
  s21_remove_matrix(&matrix);
}
END_TEST

int main(void) {
  Suite *s1 = suite_create("s21_calc_complements");
  TCase *tc1_1 = tcase_create("s21_calc_complements");
  Suite *s2 = suite_create("s21_determinant");
  TCase *tc2_1 = tcase_create("s21_determinant");
  Suite *s3 = suite_create("s21_eq_matrix");
  TCase *tc3_1 = tcase_create("s21_eq_matrix");
  Suite *s4 = suite_create("s21_inverse_matrix");
  TCase *tc4_1 = tcase_create("s21_inverse_matrix");
  Suite *s5 = suite_create("s21_mult_matrix");
  TCase *tc5_1 = tcase_create("s21_mult_matrix");
  Suite *s6 = suite_create("s21_mult_number");
  TCase *tc6_1 = tcase_create("s21_mult_number");
  Suite *s7 = suite_create("s21_sub_matrix");
  TCase *tc7_1 = tcase_create("s21_sub_matrix");
  Suite *s8 = suite_create("s21_sum_matrix");
  TCase *tc8_1 = tcase_create("s21_sum_matrix");
  Suite *s9 = suite_create("s21_transpose_matrix");
  TCase *tc9_1 = tcase_create("s21_transpose_matrix");
  SRunner *sr = srunner_create(s1);
  int nf;

  suite_add_tcase(s1, tc1_1);
  tcase_add_test(tc1_1, s21_calc_complements1);
  tcase_add_test(tc1_1, s21_calc_complements2);
  tcase_add_test(tc1_1, s21_calc_complements3);
  tcase_add_test(tc1_1, s21_calc_complements4);
  tcase_add_test(tc1_1, s21_calc_complements5);
  tcase_add_test(tc1_1, s21_calc_complements6);
  tcase_add_test(tc1_1, s21_calc_complements7);
  suite_add_tcase(s2, tc2_1);
  tcase_add_test(tc2_1, s21_determinant1);
  tcase_add_test(tc2_1, s21_determinant2);
  tcase_add_test(tc2_1, s21_determinant3);
  tcase_add_test(tc2_1, s21_determinant4);
  tcase_add_test(tc2_1, s21_determinant5);
  tcase_add_test(tc2_1, s21_determinant6);
  tcase_add_test(tc2_1, s21_determinant7);
  tcase_add_test(tc2_1, s21_determinant8);
  suite_add_tcase(s3, tc3_1);
  tcase_add_test(tc3_1, s21_eq_matrix1);
  tcase_add_test(tc3_1, s21_eq_matrix2);
  suite_add_tcase(s4, tc4_1);
  tcase_add_test(tc4_1, s21_inverse_matrix1);
  tcase_add_test(tc4_1, s21_inverse_matrix2);
  tcase_add_test(tc4_1, s21_inverse_matrix3);
  tcase_add_test(tc4_1, s21_inverse_matrix4);
  tcase_add_test(tc4_1, s21_inverse_matrix5);
  tcase_add_test(tc4_1, s21_inverse_matrix6);
  tcase_add_test(tc4_1, s21_inverse_matrix7);
  tcase_add_test(tc4_1, s21_inverse_matrix8);
  suite_add_tcase(s5, tc5_1);
  tcase_add_test(tc5_1, s21_mult_matrix1);
  tcase_add_test(tc5_1, s21_mult_matrix2);
  tcase_add_test(tc5_1, s21_mult_matrix3);
  tcase_add_test(tc5_1, s21_mult_matrix4);
  tcase_add_test(tc5_1, s21_mult_matrix5);
  suite_add_tcase(s6, tc6_1);
  tcase_add_test(tc6_1, s21_mult_number1);
  tcase_add_test(tc6_1, s21_mult_number2);
  tcase_add_test(tc6_1, s21_mult_number3);
  suite_add_tcase(s7, tc7_1);
  tcase_add_test(tc7_1, s21_sub_matrix1);
  tcase_add_test(tc7_1, s21_sub_matrix2);
  tcase_add_test(tc7_1, s21_sub_matrix3);
  tcase_add_test(tc7_1, s21_sub_matrix4);
  tcase_add_test(tc7_1, s21_sub_matrix5);
  suite_add_tcase(s8, tc8_1);
  tcase_add_test(tc8_1, s21_sum_matrix1);
  tcase_add_test(tc8_1, s21_sum_matrix2);
  tcase_add_test(tc8_1, s21_sum_matrix3);
  tcase_add_test(tc8_1, s21_sum_matrix4);
  tcase_add_test(tc8_1, s21_sum_matrix5);
  suite_add_tcase(s9, tc9_1);
  tcase_add_test(tc9_1, s21_transpose_matrix1);
  tcase_add_test(tc9_1, s21_transpose_matrix2);
  tcase_add_test(tc9_1, s21_transpose_matrix3);
  tcase_add_test(tc9_1, s21_transpose_matrix4);
  tcase_add_test(tc9_1, s21_transpose_matrix5);
  tcase_add_test(tc9_1, s21_transpose_matrix6);
  tcase_add_test(tc9_1, s21_transpose_matrix7);

  srunner_add_suite(sr, s2);
  srunner_add_suite(sr, s3);
  srunner_add_suite(sr, s4);
  srunner_add_suite(sr, s5);
  srunner_add_suite(sr, s6);
  srunner_add_suite(sr, s7);
  srunner_add_suite(sr, s8);
  srunner_add_suite(sr, s9);

  srunner_set_fork_status(sr, CK_NOFORK);

  srunner_run_all(sr, CK_ENV);
  nf = srunner_ntests_failed(sr);
  srunner_free(sr);

  return nf == 0 ? 0 : 1;
}
